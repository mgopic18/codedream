...
export function Scene() {
    ...
    }, [size, gl, fbo1]) // Removed fbo1 from dependencies to avoid loop, though it's likely fine. The real issue is usually texture params.

    // Create off-screen scene for simulation
    const { simulationScene, simulationCamera } = useMemo(() => {
        ...
        scene.add(mesh)
        return { simulationScene: scene, simulationCamera: camera }
    }, [])
    ...
    useFrame((state, delta) => {
        ...
        if (pointsRef.current) {
            pointsRef.current.rotation.y += delta * 0.06 // approx 0.001 * 60
            pointsRef.current.rotation.x += delta * 0.03 // approx 0.0005 * 60
        }
    })
